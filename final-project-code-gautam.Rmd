...
d <- d.clean.survey[,c("gender","race","income","degree","Y_1")]
d <- na.omit(d)

gender_list <- c(unique(d[, gender]))
race_list <- c(unique(d[, race]))
income_list <- c(unique(d[,income]))
for(item in gender_list){
  print(mean(d[gender==item,Y_1]))
}

for(item1 in gender_list){
  for(item2 in race_list) {
    for(item3 in income_list) 
      cat(item1, item2, item3, mean(d[gender==item1 & race==item2 & income==item3,.N]),mean(d[gender==item1 & race==item2 & income==item3,Y_1]),"\n")
  }
}

mean(d[gender=="Male" & race=="White / Caucasian",Y_1])
mean(d[gender=="Female" & race=="Black or African American",Y_1])

m1 <- d[, lm(Y_1 ~ gender)]
m2 <- d[, lm(Y_1 ~ gender + race + gender * race)]
...
```
```{r}
mean(d[income=="Less than $10,000",Y_1])
